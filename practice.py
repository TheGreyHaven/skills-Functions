...
def print_sign(n):
    """Prints weather n is higher or lower than 0"""
    if n >= 1:
        print "Higher than 0"
    elif n < 0:
        print "Lower than 0"
...
def is_divisible_by_three(n):
    """Returns True if n is evenly divisible by three or False if not"""
    return n % 3 is 0
...
#I read this line of code a few months ago and I kept it in my head but I didn't create it
...
def total_meal_price(meal_price, tip_percentage = .15):
    """Calculates the cost of the meal plus tip"""
...
def sign_and_parity(n):
    """Returns weather an intiger is Positive, Negative, Even or Odd"""
    describe_n = []
    if n >= 0:
        pos_or_neg = "Positive"
    else:
    ...
    describe_n.extend([pos_or_neg, even_or_odd])
    
    return describe_n

describing_n = sign_and_parity(5)

sign = describing_n[0]
parity = describing_n[1]
...
def full_title(name, job_title = "Engineer"):
    """returns job_title and name as one string"""
    return job_title + " " + name

def write_letter(job_title, name, SENDER_NAME):
    """calls on full_title function to write a letter"""
    title_name = full_title("Jane Hacks", "Hacker")
    print "Dear {}, I think you are amazing! Sincerely, {}".format(title_name, SENDER_NAME)
